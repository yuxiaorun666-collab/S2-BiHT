...
DATA_DIR = 'Submission_Dataset/SEED'
BEST_MODEL_DIR = './seed_model'
...
class S2_BiHT_V2_DANN(nn.Module):
    def __init__(self, num_classes=3, embed_dim=64, num_heads=4):
        super().__init__()
...
def get_test_loader_for_subject(sid):
    ...
    raw_trials = [mat[f'de_LDS{k}'] for k in range(1, 16) if f'de_LDS{k}' in mat]
    if not raw_trials: return None

    norm_trials = apply_session_zscore(raw_trials)

    for t_d, t_l in zip(norm_trials, SEED_LABELS_MAPPED):
...
if __name__ == '__main__':
    print("=" * 60)
    print("S2-BiHT (SEED) Evaluation Script")
    ...
    print("\nEvaluating 15 subjects from SEED dataset...\n")
